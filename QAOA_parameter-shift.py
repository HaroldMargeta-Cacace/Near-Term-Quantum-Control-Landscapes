...
def parameter_shift_gradient(f, x, shift=np.pi/2):
    gradient = np.zeros_like(x)
    for i in range(len(x)):
        shift_vector = np.zeros_like(x)
        shift_vector[i] = shift
        f_plus = f(x + shift_vector)
        f_minus = f(x - shift_vector)
...
def train_QAOA(f, N, p):
  ...
  stored_parameters = [input.copy()]

  def callback_function(curr_param):
    stored_parameters.append(curr_param)
  
  start_time = time.time()
  res = scipy.optimize.minimize(f, input, method='BFGS', 
                                jac = lambda x: parameter_shift_gradient(f, x), 
                                callback=callback_function, options={'maxiter': 1000})
...
graph_dicts = load_graphs('Vertex_and_Edge-Transitive_Graphs')
...
df.to_csv('vertex_and_edge_transitive_p1-7.csv')
df.to_pickle('vertex_and_edge_transitive_p1-7.pkl')
