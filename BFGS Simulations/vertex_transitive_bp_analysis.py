# -*- coding: utf-8 -*-
"""Copy of Vertex_Transitive Test_BP_Analysis.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1DNei94ANKP9HBG1TfOuj0_f2X-2yafA6
"""
...
df = pd.read_pickle("/content/drive/MyDrive/Concatenated_Vertex_Transitive_N4-20_p1-4.pkl")

# sampled_df = pd.DataFrame()
# unique_p_values = df['p'].unique()

# for p_val in unique_p_values:
#     df_p = df[df['p'] == p_val].copy()
#     df_p_filtered_N = df_p[df_p['N'] >= 8].copy()

#     if df_p_filtered_N.empty:
#         print(f"No data for p={p_val} with N >= 8. Skipping.")
#         continue

#     # Determine the minimum number of data points for N >= 8
#     min_samples_per_N = df_p_filtered_N['N'].value_counts().min()

#     if min_samples_per_N == 0:
#         print(f"No data points for at least one N >= 8 for p={p_val}. Skipping.")
#         continue

#     print(f"For p={p_val}, minimum samples per N (>=8) is: {min_samples_per_N}")

#     # Sample min_samples_per_N points for each N
#     sampled_p_df = df_p_filtered_N.groupby('N').apply(lambda x: x.sample(n=min_samples_per_N, random_state=42)).reset_index(drop=True)
#     sampled_df = pd.concat([sampled_df, sampled_p_df], ignore_index=True)

# print("\nSampled DataFrame created.")
# display(sampled_df.head())
# display(sampled_df['N'].value_counts().sort_index())
# display(sampled_df['p'].value_counts().sort_index())

#df = sampled_df.copy()

base_fname = "concat_vertex_trans"
...
def run_complete_analysis(qaoa_df, base_name, gfeatures=features, regvars=['mean_grad_variance', 'loss_variance']):
            ...
            for regvar in regvars:
                for feat in feature_cols:
                    results_df = analyze_feature_regressions(
                        ...
                        feat,
                        regvar,
                        min_cluster_size=max(5, len(df_p) // 50),
                        min_points=max(2, len(df_p) // 40),
                        min_samples=max(2, len(df_p) // 30)
                    )
...
plot_N_vs_mean_grad_variance_with_fit(df, corr_feat_df, top_n=20)
...
def plot_ridge_plots_by_p_and_feature(corr_feat_df_grads, bins=80, smooth_sigma=1, spacing_factor=1.2, min_fig_height=5, bandwidth_factor=None):
    """
    ...
    Args:
        ...
        bandwidth_factor (float, optional): A factor to multiply the automatically
                                            estimated bandwidth by. Can be used to
                                            manualy adjust bandwidth. Defaults to None.
    """
    ...
    for (p_val, feature_name), group_df in corr_feat_df_grads.groupby(['p', 'feature']):
        ...
        for i, row in group.iterrows():
            ...
            if n_points == 0:
                print(f"  Skipping plotting for cluster '{cluster_label}' (p={p_val}, feature='{feature_name}') due to n_points == 0.")
                continue # Should have been filtered out, but just in case

            if not vals.size > 0:
                 print(f"  Skipping plotting for cluster '{cluster_label}' (p={p_val}, feature='{feature_name}') due to empty feature_values_list.")
                 continue
            ...
            try:
                # Try to use KDE if there's enough unique data points and variability
                if np.unique(vals).size > 1 and n_points > 1 and np.std(vals) > 1e-9: # Added std check
                    try:
                        # Attempt default bandwidth
                        kde = gaussian_kde(vals)
                        if bandwidth_factor is not None:
                            # Apply bandwidth factor if provided
                            kde.set_bandwidth(bw_method=kde.factor * bandwidth_factor)

                    except LinAlgError as linalg_e:
                        print(f"  LinAlgError in KDE for cluster '{cluster_label}' (p={p_val}, feature='{feature_name}'): {linalg_e}")
                        print(f"  Cluster Data (first 5 vals): {vals[:5]}, n_points: {n_points}, unique_size: {np.unique(vals).size}, std: {np.std(vals)}")
                        # Fallback to a simple fixed bandwidth (could be tuned)
                        try:
                             # A simple fallback, could be tuned
                             # Use a fraction of the standard deviation or IQR
                             fallback_bandwidth = np.std(vals) * 0.5 # Example fallback bandwidth
                             if fallback_bandwidth < 1e-9 or np.isnan(fallback_bandwidth):
                                  # If std is zero or NaN, fallback to a tiny value or skip KDE
                                  raise ValueError("Std deviation too small for fallback bandwidth") # Jump to outer exception handling

                             kde = gaussian_kde(vals, bw_method=fallback_bandwidth)

                             if bandwidth_factor is not None:
                                # Apply bandwidth factor even to fallback bandwidth
                                kde.set_bandwidth(bw_method=kde.factor * bandwidth_factor)

                        except Exception as inner_e:
                             print(f"  Fallback bandwidth failed for cluster '{cluster_label}' (p={p_val}, feature='{feature_name}'): {inner_e}. Skipping KDE and falling back to point/line plotting.")
                             # If even fallback bandwidth fails, skip KDE and use alternative plotting
                             raise # Re-raise to trigger outer exception block for fallback plotting
                    ...
                    # Handle cases with single unique value, single point, or no variability
                    print(f"  Plotting fallback for cluster '{cluster_label}' (p={p_val}, feature='{feature_name}'): unique_size={np.unique(vals).size}, n_points={n_points}, std={np.std(vals)}")
                    if n_points > 0:
                        single_val = vals[0] # Assuming all values are the same if unique size is 1
                        if np.unique(vals).size == 1 and n_points > 1:
                            ...
                            ax.text(vals[0], vertical_offset + 0.5,
                                    ...
                                    va='center', ha='left', fontsize=8, color='black')
                        else: # n_points > 0, unique values > 1, but std is ~0 (KDE might fail)
                             # Fallback to plotting raw points and annotation
                ...
                print(f"  General Error processing cluster '{cluster_label}' (p={p_val}, feature='{feature_name}'): {e}. Falling back to plotting points/line.")
                print(f"  Cluster Data (first 5 vals): {vals[:5]}, n_points: {n_points}, unique_size: {np.unique(vals).size}, std: {np.std(vals)}")

                 # Fallback to plotting raw points or a line if any other error occurs (e.g., KDE fails for other reasons)
                if n_points > 0:
                     if np.unique(vals).size == 1:
                         ...
                         bin_idx = np.digitize([single_val], bins_edges)[0] - 1
                         if 0 <= bin_idx < len(bins_centers):
                              ax.plot([bins_centers[bin_idx], bins_centers[bin_idx]], [vertical_offset, vertical_offset + 1.0], color=color, lw=3)
                              annotation_text = f'r={pearson_r:.2f}'
                              ax.text(bins_centers[bin_idx], vertical_offset + 0.5,
                                      annotation_text,
                                      va='center', ha='left', fontsize=8, color='black')
                     else:
                         # Plot raw points if there's variability or it's a single point
    ...
    if df_all_p.empty:
        ...
        if len(mean_variance_by_N_all_p) < 2:
            ...
            with plt.style.context('seaborn-v0_8-paper'):
                ...
                cbar_all_p.set_label('% of total data at N')
...
# Step 1: Extract percent error values for clusters
# Reuse the extract_cluster_metrics function, specifying 'percent_error' as the metric
corr_feat_df_percent_error = extract_cluster_metrics(df, corr_feat_df_vals, metric_columns=['percent_error'])

display(corr_feat_df_percent_error.head())

# Step 2: Compute cluster percent error stats
# Reuse the calculate_cluster_stats function for 'percent_error'
corr_feat_df_percent_error = calculate_cluster_stats(corr_feat_df_percent_error, metric_columns=['percent_error'])

# Display the head of the updated DataFrame with the new stats
display(corr_feat_df_percent_error.head())
...
unique_p_values = corr_feat_df_percent_error['p'].unique()
metric_to_plot = 'percent_error' # We are plotting percent error

for p_val in unique_p_values:
    print(f"Plotting p={p_val}, Metric={metric_to_plot}")

    # Filter data for the current p-value and metric
    metric_mean_col = f'{metric_to_plot}_mean'
    metric_std_col = f'{metric_to_plot}_std'

    filtered_group = corr_feat_df_percent_error[
        (corr_feat_df_percent_error['p'] == p_val) &
        corr_feat_df_percent_error[metric_mean_col].notna()
    ].copy()

    if filtered_group.empty:
        print(f"  No valid data for p={p_val}, Metric={metric_to_plot}. Skipping plotting.")
        continue
    ...
    # Perform Exponential Fit
    try:
        # Ensure there are enough data points and variability for the fit
        if len(x_plot_data) > 2 and np.std(y_plot_data) > 0:
            popt_exp, pcov_exp = curve_fit(exponential_func, x_plot_data, y_plot_data, maxfev=5000)
            A_exp, B_exp = popt_exp
            # Calculate R-squared for Exponential Fit
            residuals_exp = y_plot_data - exponential_func(x_plot_data, A_exp, B_exp)
            ...
            r_squared_exp = 1 - (ss_res_exp / ss_tot_exp) if ss_tot_exp > 0 else np.nan
             ...
             A_exp, B_exp = np.nan, np.nan
             r_squared_exp = np.nan
             print(f"  Not enough data or variability for Exponential fit for p={p_val}, Metric={metric_to_plot}.")

    except RuntimeError:
        A_exp, B_exp = np.nan, np.nan
        r_squared_exp = np.nan
        print(f"  Exponential fit failed for p={p_val}, Metric={metric_to_plot}.")


    # Perform Linear Fit
    try:
        # Ensure there are enough data points for the fit
        if len(x_plot_data) > 1:
            slope_linear, intercept_linear, r_linear, p_value_linear, std_err_linear = linregress(x_plot_data, y_plot_data)
            # R-squared for Linear Fit is simply r_linear**2
            r_squared_linear = r_linear**2
        else:
             slope_linear, intercept_linear, r_linear, r_squared_linear = np.nan, np.nan, np.nan, np.nan
             print(f"  Not enough data for Linear fit for p={p_val}, Metric={metric_to_plot}.")


    except ValueError:
        slope_linear, intercept_linear, r_linear, r_squared_linear = np.nan, np.nan, np.nan, np.nan
        print(f"  Linear fit failed for p={p_val}, Metric={metric_to_plot}.")
    ...
    plt.xlabel('Pearson r (Mean Grad Variance vs N)')
    plt.ylabel(f'Mean {metric_to_plot.replace("_", " ").title()}')
    plt.title(f'Mean {metric_to_plot.replace("_", " ").title()} vs Pearson r (Mean Grad Variance vs N) for p={p_val}')
    plt.legend()
    plt.grid(True, which="both", ls="--", c='0.7')
    plt.show()
...
# Check for rows with empty feature_values_list in corr_feat_df_vals
empty_feature_value_clusters = corr_feat_df_vals[
    corr_feat_df_vals['feature_values_list'].apply(lambda x: len(x) == 0)
]

if not empty_feature_value_clusters.empty:
    print("Clusters with empty feature_values_list:")
    display(empty_feature_value_clusters[['feature', 'p', 'cluster_label', 'n_points']])
else:
    print("No clusters with empty feature_values_list found in corr_feat_df_vals.")

import pandas as pd
import numpy as np

def compute_and_rank_pearson_stats(corr_feat_df, p_values=[1, 2, 3, 4]):
    """
